...
app1 = dash.Dash(external_stylesheets=[dbc.themes.BOOTSTRAP]) # [dbc.themes.BOOTSTRAP]
project_name = None

#Declaring function for project
def load_data():
...
def create_app_ui():
    ...
    html.H1('CDR Analysis with Insights', id='Main_title',style={'backgroundColor':'yellow','align':'center','textAlign':'center',"color":"red"}),
...
def count_devices(data):
    ...
    for var in reformed_data["UserDeviceType"]:
        ...
        if re.search("Windows",var):
            device_dict["Windows"]+=1
        if re.search("iphone",var):
            device_dict["iphone"]+=1
        if re.search("Android",var):
            device_dict["Android"]+=1
        if re.search( "Mac",var):
            device_dict[ "Mac"]+=1
        if re.search( "Yealink",var):
...
def update_app_ui(Tabs, start_date, end_date, group, report_type,device_date,service_date):
    ...
    if Tabs == "tab-1":
        ...
        page_size=5,
        page_action='native',
        style_header={'backgroundColor': 'rgb(0, 0, 0)'},
        style_cell={
...
def main():
    ...
    global app1, project_name
    project_name = "Project on CDR Analysis"
